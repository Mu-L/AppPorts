  ...
  "zh-Hans": {
    ...
    clauses: [
      ...
      },
      {
        ...
        tail: ["您可以随时删除上述文件及本软件，以清除全部本地数据。"],
      },
      {
        title: "第八条  诊断包",
        paras: ["“导出诊断包”功能仅在您主动点击后执行，并且："],
        items: [
      ...
      {
        title: "第十八条  联系方式",
        paras: ["有关本政策或个人信息保护的任何疑问、意见或投诉，可通过下列方式与我们联系："],
        contact: { label: "电子邮箱", value: "a@shimoko.com", href: "mailto:a@shimoko.com" },
        tail: ["我们将在收到您的请求后十五个工作日内予以答复。"],
      },
    ],
  },

  en: {
    title: "AppPorts Privacy Policy",
    marker: "decimal",
    meta: [
    ...
    clauses: [
      {
        title: "1. Definitions",
        paras: ["For the purposes of this Policy:"],
        items: [
      ...
      {
        title: "4. Personal Information We Collect and Use",
        paras: [
      ...
      {
        title: "5. Information Read Locally",
        paras: [
      ...
      {
        title: "6. Network Access and Recipients",
        paras: [
      ...
      },
      {
        ...
        tail: ["You may delete these files and the Software itself at any time to erase all local data."],
      },
      {
        title: "8. Diagnostic Package",
        paras: ["The “export diagnostic package” function runs only when you click it, and:"],
        items: [
      ...
      {
        title: "9. System Permissions",
        paras: ["To provide the functions described above, the Software may involve the following system permissions:"],
        items: [
      ...
      {
        title: "18. Contact",
        paras: ["For any question, comment, or complaint about this Policy or about the protection of personal information, you may contact us as follows:"],
        contact: { label: "Email", value: "a@shimoko.com", href: "mailto:a@shimoko.com" },
        tail: ["We will respond to your request within fifteen business days of receipt."],
      },
    ],
  },
};

const t = computed(() => policies[props.locale] ?? policies["zh-Hans"]);
</script>
...
<template>
  <article class="policy">
    <header class="masthead">
      ...
      <dl class="doc-meta">
        <div v-for="row in t.meta" :key="row[0]" class="meta-row">
          <dt>{{ row[0] }}</dt>
          <dd>{{ row[1] }}</dd>
        </div>
      </dl>
    ...
    <p v-for="(para, index) in t.preamble" :key="'pre-' + index" class="para">{{ para }}</p>

    <section v-for="clause in t.clauses" :key="clause.title" class="clause">
      <h2 class="clause-title">{{ clause.title }}</h2>
      <p v-for="(para, index) in clause.paras" :key="'p-' + index" class="para">{{ para }}</p>
      <ol v-if="clause.items" class="items" :class="t.marker">
        <li v-for="(item, index) in clause.items" :key="'i-' + index">{{ item }}</li>
      </ol>
      <p v-for="(para, index) in clause.tail" :key="'t-' + index" class="para">{{ para }}</p>
      <p v-if="clause.contact" class="contact">
        <span class="contact-label">{{ clause.contact.label }}</span>
        <a :href="clause.contact.href">{{ clause.contact.value }}</a>
      </p>
    </section>
  </article>
</template>
