...
微信这类应用对容器路径、签名状态和本地数据一致性比较敏感。**不要用重签名来尝试修复**：Ad-hoc 重签名会抹掉微信的沙盒授权与 Team ID，而腾讯的原始证书不在本机钥匙串，无法还原。详见[容器数据与签名身份](/datamigrae/container-identity)。

建议按以下顺序处理：

1. 完全退出微信。
2. 在数据目录管理中执行「恢复」，把数据迁回本地（**不要**勾选重签名）。
3. 确认容器里的账号目录已经是真实目录，而不是指向外部存储的符号链接。
4. 如果仍然打不开，从官网重新下载安装微信覆盖本地副本，以恢复原始签名。
5. 如需反馈问题，请导出诊断包并附上微信版本、macOS 版本和迁移的数据目录类型。

迁移聊天记录、虚拟机、数据库等高价值数据前，建议先做独立备份。

### 升级 macOS 后应用突然打不开，是之前的数据迁移导致的吗？

可能有关，但通常**不是数据被迁移坏了，而是签名身份的问题**。

如果这个应用在迁移 `~/Library/Containers/` 或 `~/Library/Group Containers/` 数据时执行过 Ad-hoc 重签名，它会失去沙盒授权，以普通进程身份访问自己的容器。较旧的 macOS 通常会放行，系统大版本升级后校验变严，访问就会被拒绝，表现为双击无反应、进程闪退。

这种情况下迁回数据、再次重签名都无效，需要从官方渠道重新安装该应用以恢复原始签名。完整的判断方法和修复步骤见[容器数据与签名身份](/datamigrae/container-identity)。
